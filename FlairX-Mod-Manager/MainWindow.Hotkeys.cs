    ...
    public sealed partial class MainWindow : Window
    {
        ...
        private async Task ExecuteShuffleActiveModsHotkeyInternal()
        {
            try
            {
                ...
                foreach (var categoryDir in Directory.GetDirectories(modsPath))
                {
                    ...
                    {
                        // Keep excluded category mods as they are
                ...
                // Step 2: Activate 1 random mod from each category (except excluded)
                foreach (var categoryDir in Directory.GetDirectories(modsPath))
                {
                    if (!Directory.Exists(categoryDir)) continue;
        ...
        private async Task ExecuteDeactivateAllModsHotkeyInternal()
        {
            try
            {
                ...
                // Get shuffle excluded categories (these will be excluded from deactivation too)
                var gameTag = SettingsManager.GetGameTagFromIndex(SettingsManager.Current.SelectedGameIndex);
                var excludedCategories = !string.IsNullOrEmpty(gameTag) 
                    ? SettingsManager.GetShuffleExcludedCategories(gameTag) 
                    : new List<string>();
        ...
        private void OnGlobalGamepadButtonPressed(object? sender, GamepadButtonEventArgs e)
        {
            ...
            {
                Logger.LogInfo($"Gamepad combo {configuredCombo} detected - sending keyboard hotkey for overlay");
                // Simulate the configured keyboard hotkey instead of calling ToggleOverlayWindow directly.
                // This routes through WM_HOTKEY → WndProc which has "foreground love" → SetForegroundWindow works.
                SimulateOverlayHotkey();
                _heldButtons.Clear();
            }
        }
        ...
        /// <summary>
        /// Simulates the configured keyboard hotkey for overlay toggle via SendInput.
        /// This triggers WM_HOTKEY in WndProc which has "foreground love" from the hotkey event,
        /// allowing SetForegroundWindow to work and give the overlay proper focus.
        /// </summary>
        private void SimulateOverlayHotkey()
        {
            try
            {
                var hotkeyStr = SettingsManager.Current.ToggleOverlayHotkey ?? "Alt+W";
                var parts = hotkeyStr.Split('+');
                if (parts.Length == 0) return;

                var inputs = new System.Collections.Generic.List<INPUT>();

                // Key down for each part
                foreach (var part in parts)
                {
                    ushort vk = ParseVirtualKey(part.Trim());
                    if (vk == 0) continue;
                ...
                {
                    ushort vk = ParseVirtualKey(parts[i].Trim());
                    if (vk == 0) continue;
                    inputs.Add(new INPUT
                    {
                        type = INPUT_KEYBOARD,
                ...
                if (inputs.Count > 0)
                {
                    ...
                    Logger.LogInfo($"SimulateOverlayHotkey: sent {hotkeyStr}");
                }
            }
            catch (Exception ex)
            {
                Logger.LogError("SimulateOverlayHotkey failed", ex);
            }
        }

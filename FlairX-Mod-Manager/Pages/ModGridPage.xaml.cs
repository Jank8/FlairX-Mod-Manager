...
namespace FlairX_Mod_Manager.Pages
{
    ...
    public sealed partial class ModGridPage : Page
    {
        ...
        // New: tile hover handlers - show hover state and play small scale animation to emulate press-on-hover
        private void TileButton_PointerEntered(object sender, PointerRoutedEventArgs e)
        {
            try
            {
                ...
                {
                    // Do not set mod.IsHovered here so activation (heart) button only reacts to its own pointer events
                    // if (btn.Tag is ModTile mod)
                    // {
                    //     mod.IsHovered = true;
                    // }

                    // Ensure RenderTransform exists and is a ScaleTransform
                    ...
                    {
                        To = 1.1, // Enlarge by 10%
                        Duration = new Duration(TimeSpan.FromMilliseconds(300)),
                        EasingFunction = new Microsoft.UI.Xaml.Media.Animation.ElasticEase { EasingMode = Microsoft.UI.Xaml.Media.Animation.EasingMode.EaseOut, Oscillations = 1, Springiness = 6 }
                    };
        ...
        private void TileButton_PointerExited(object sender, PointerRoutedEventArgs e)
        {
            try
            {
                if (sender is Button btn)
                {
                    // Do not clear mod.IsHovered here; ModActiveButton handlers control heart hover state
                    // if (btn.Tag is ModTile mod)
                    // {
                    //     mod.IsHovered = false;
                    // }

...
namespace FlairX_Mod_Manager.Pages
{
    ...
    public sealed partial class ModGridPage : Page
    {
        ...
        // New: tile hover handlers - show subtle hover effect on inner Border to avoid interfering with grid scaling
        private readonly Dictionary<Border, SpriteVisual> _tileShadowMap = new Dictionary<Border, SpriteVisual>();
        ...
        private void TileButton_PointerEntered_Shadow(object sender, PointerRoutedEventArgs e)
        {
            try
            {
                if (sender is Button btn)
                {
                    ...
                    // If shadow already created, skip
                    if (_tileShadowMap.ContainsKey(tileBorder))
                        return;

                    // Ensure ActualSize is available
                    if (tileBorder.ActualWidth <= 0 || tileBorder.ActualHeight <= 0)
                    {
                        // Defer creation until layout happened
                        tileBorder.Loaded += TileBorder_Loaded_CreateShadow;
                        return;
                    }

                    CreateAndShowShadow(tileBorder);
                }
            }
            catch (Exception ex)
            {
        ...
        private void TileBorder_Loaded_CreateShadow(object? sender, RoutedEventArgs e)
        {
            if (sender is Border tileBorder)
            {
                tileBorder.Loaded -= TileBorder_Loaded_CreateShadow;
                if (!_tileShadowMap.ContainsKey(tileBorder))
                {
        ...
        private void CreateAndShowShadow(Border tileBorder)
        {
            try
            {
                var visual = ElementCompositionPreview.GetElementVisual(tileBorder);
                var compositor = visual.Compositor;

                var dropShadow = compositor.CreateDropShadow();
                // Leave color default (black) to avoid platform-specific color type issues
                dropShadow.BlurRadius = 24f;
                dropShadow.Offset = new Vector3(0, 10, 0);
                dropShadow.Opacity = 0f; // start invisible

                var shadowVisual = compositor.CreateSpriteVisual();
                shadowVisual.Shadow = dropShadow;
                shadowVisual.Size = new Vector2((float)tileBorder.ActualWidth, (float)tileBorder.ActualHeight);

                // Attach shadow visual as a child of the element (it will render behind the content)
                ElementCompositionPreview.SetElementChildVisual(tileBorder, shadowVisual);

                _tileShadowMap[tileBorder] = shadowVisual;

                // Animate shadow opacity and offset to create 'lift' effect
                var opacityAnim = compositor.CreateScalarKeyFrameAnimation();
                opacityAnim.InsertKeyFrame(1f, 0.32f);
                ...
                offsetAnim.InsertKeyFrame(1f, new Vector3(0, -8, 0));
                offsetAnim.Duration = TimeSpan.FromMilliseconds(220);
                shadowVisual.StartAnimation("Offset", offsetAnim);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"CreateAndShowShadow error: {ex.Message}");
            }
        }
        ...
        private async void TileButton_PointerExited_Shadow(object sender, PointerRoutedEventArgs e)
        {
            try
            {
                if (sender is Button btn)
                {
                    ...
                    if (!_tileShadowMap.TryGetValue(tileBorder, out var shadowVisual))
                        return;

                    var compositor = shadowVisual.Compositor;
                    var compShadow = shadowVisual.Shadow;
                    if (compShadow == null)
                    {
                        // Remove immediately if no shadow
                        ElementCompositionPreview.SetElementChildVisual(tileBorder, null);
                        _tileShadowMap.Remove(tileBorder);
                        return;
                    }

                    // Animate opacity to 0 and offset back to normal
                    var opacityAnim = compositor.CreateScalarKeyFrameAnimation();
                    opacityAnim.InsertKeyFrame(1f, 0f);
                    opacityAnim.Duration = TimeSpan.FromMilliseconds(180);
                    compShadow.StartAnimation("Opacity", opacityAnim);

                    var offsetAnim = compositor.CreateVector3KeyFrameAnimation();
                    offsetAnim.InsertKeyFrame(1f, new Vector3(0, 10, 0));
                    offsetAnim.Duration = TimeSpan.FromMilliseconds(180);
                    shadowVisual.StartAnimation("Offset", offsetAnim);

                    // Wait for animation to finish then remove visual
                    await Task.Delay(200);
                    if (_tileShadowMap.ContainsKey(tileBorder))
                    {
                        ElementCompositionPreview.SetElementChildVisual(tileBorder, null);

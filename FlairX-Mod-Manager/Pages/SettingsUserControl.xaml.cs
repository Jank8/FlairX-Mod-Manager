...
namespace FlairX_Mod_Manager.Pages
{
    public sealed partial class SettingsUserControl : UserControl
    {
        ...
        {
            // For RenameOnly mode, just copy preview.jpg if it doesn't exist
            if (mode == OptimizationMode.RenameOnly)
            {
                // Categories in RenameOnly mode: just ensure preview.jpg exists, no optimization
                return;
            }
            
            // For Rename mode, files already have standard names
            if (mode == OptimizationMode.Rename)
            {
                // Categories already have preview.jpg, no need to do anything
                return;
            }
        ...
        public static void ProcessModPreviewImagesStatic(string modDir, OptimizationMode mode = OptimizationMode.Full)
        {
            try
            {
                // For RenameOnly mode, just copy files with standard names (no optimization)
                if (mode == OptimizationMode.RenameOnly)
                {
                ...
                // For Rename mode, files already have standard names, just return
                if (mode == OptimizationMode.Rename)
                {
                    // Files are already renamed to standard names (preview.jpg, preview-01.jpg, etc.)
                    // Thumbnails will be generated by the optimization process
                    return;
                }
